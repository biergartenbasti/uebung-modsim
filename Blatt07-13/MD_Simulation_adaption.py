...
MD_target='LiPSCl_Trajectories'
plot_title='LiPSCl Bulk Diffusion'
...
for traj_file in trajectories_list:
    ...
    # Extract temperature from filename - adjust this based on your actual file naming pattern
    ...
    axes[0, 0].set_xlabel(r'$\Delta t$/fs')
    ...
    axes[0, 1].set_xlabel(r'$\Delta t$/fs')
    ...
    axes[1, 0].set_xlabel(r'$\Delta t$/fs')
    ...
    print(f"{temp} K; D = {diff.D.n:.2e} ± {(diff.D.ci()[1] - diff.D.ci()[0])/2:.2e}")
...
#%% Arrhenius analysis
s = StandardArrhenius(temps, D, bounds=((1E-3,1),(1e-10,1)))
s.max_likelihood('mini')
s.mcmc(n_samples=1000)
...
# Text box for parameter estimates
...
# Calculate diffusion coefficient at 300K using correct Arrhenius equation
D300K_value = s.variable_medians[1] * np.exp(-s.variable_medians[0] / (kB_eV * 300))
D300K = np.format_float_scientific(D300K_value, precision=3)
...
# Calculate Median Absolute Deviation (MAD) for both parameters
mad_ea = np.median(np.absolute(s.variables[0] - np.median(s.variables[0])))
mad_a = np.median(np.absolute(s.variables[1] - np.median(s.variables[1])))
print("Median Absolute Deviation of Ea / eV is: "+np.format_float_scientific(mad_ea))
print("Median Absolute Deviation of A / cm2 s-1 is: "+np.format_float_scientific(mad_a))
...
with open(MD_target+'/MD_Parameters_kinisi.txt', 'w') as f:
    ...
    f.write("Uncertainty Estimates (MAD):\n")
    f.write("Median Absolute Deviation of Ea / eV is: "+np.format_float_scientific(mad_ea)+'\n')
    f.write("Median Absolute Deviation of A / cm2 s-1 is: "+np.format_float_scientific(mad_a)+'\n\n')
    
    f.write("Predictions:\n")
    f.write("Using median values, D / cm2 s-1 at 300K is: "+D300K+'\n')
    f.write("Arrhenius equation used: D = A * exp(-Ea / (kB * T))\n")
    f.write("where kB = "+str(kB_eV)+" eV/K\n")

#%% Plot Arrhenius function
fig2,ax2=plt.subplots(figsize=(10,8))

# Plot data points with error bars
plt.errorbar(1000/s.x, s.y.n, s.y.ci(), marker='o', ls='', color='k', 
             markersize=8, capsize=5, capthick=2, label='Data points', zorder=10)

# Create temperature range for smooth Arrhenius line
T_range = np.linspace(300, max(temps)*1.1, 200)
# Calculate diffusion coefficients using Arrhenius equation: D = A * exp(-Ea / (kB * T))
D_arrhenius = s.variable_medians[1] * np.exp(-s.variable_medians[0] / (kB_eV * T_range))

# Plot Arrhenius fit line
plt.plot(1000/T_range, D_arrhenius, color='red', linewidth=2, 
         label='Arrhenius fit (median params)', zorder=5)

# Formatting
plt.yscale('log')
plt.xlabel('$1000T^{-1}$/K$^{-1}$', fontsize=14)
plt.ylabel('$D$/cm$^2$s$^{-1}$', fontsize=14)
plt.title(plot_title, fontsize=16)
plt.grid(True, alpha=0.3)
plt.legend(fontsize=12)

# Enhanced text box with more information
textbox_content = (f"Median Parameters:\n"
                  f"Ea = {s.variable_medians[0]:.3f} eV\n"
                  f"A = {s.variable_medians[1]:.2e} cm²s⁻¹\n"
                  f"D₃₀₀ₖ = {D300K} cm²s⁻¹\n"
                  f"R² = {getattr(s, 'r_squared', 'N/A')}")

ax2.text(0.02, 0.98, textbox_content, transform=ax2.transAxes, fontsize=12,
         verticalalignment='top', ha='left', bbox=props)

plt.tight_layout()
plt.savefig(MD_target+'/ArrheniusPlot_kinisi'+plot_title+'.png',dpi=1000, bbox_inches='tight')
plt.show()
